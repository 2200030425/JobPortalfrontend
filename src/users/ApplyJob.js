...
import tips from "../images/addjob.jpg";
...
export default function ApplyJob() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (firstName.length <= 3 || !/^[A-Za-z]+$/.test(firstName)) {
      alert("First name must be more than 3 characters and contain only letters.");
      return;
    }

    if (lastName.length <= 3 || !/^[A-Za-z]+$/.test(lastName)) {
      alert("Last name must be more than 3 characters and contain only letters.");
      return;
    }

    // Validate if first name and last name are not the same
